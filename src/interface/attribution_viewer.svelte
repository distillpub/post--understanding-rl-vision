<script>
  ...
  export let chart_quantile_to_overflow;
  export let chart_max_overflow;
  export let chart_fixed_upper_value;
  export let chart_fixed_lower_value;

  /* const attribution_label = function(attribution_kind) {
   *   if (attribution_kind.type === "v"){
   *     return "Value function attribution";
   *   }
   *   else if (attribution_kind.type === "action") {
   *     return action_htmls[attribution_kind.data] + " logit";
   *   }
   *   else if (attribution_kind.type === "action_group") {
   *     let actions = action_groups[attribution_kind.data];
   *     return "[ " + actions.map(action => action_htmls[action]).join(" | ") + " ] logit sum";
   *   }
   *   else {
   *     return "";
   *   }
  ...
  const load_attribution_totals = function(totals_jsons, callback, namespace) {
    if (totals_jsons !== null) {
      ...
      json_load(urls, json_namespace_prefix + "_" + namespace, json_preloaded).then(callback);
    }
    return null;
  };
...
<table style="display: {show_attribution_selector ? 'table' : 'none'};">
  <tr>
    {#if attribution_policy}
      ...
      <AttributionSelector
        bind:type={attribution_kinds[0].type}
        bind:data={attribution_kinds[0].data}
    ...
    {#if attribution_kind !== null}
      ...
      <tr>
        {#each attribution_options[kind_index] as options, options_index}
          <td style="width: 33.33%; min-width: {css_multiply(video_width, 3/4)}; max-width: {video_width};">
            {#if options !== null}
              <div style="display: inline-block; border: 1px solid gray; height: 0px; padding-bottom: 100%; width: 100%;">
                <Screen
                  image_dir={""}
...
{#if show_chart}
  <Chart
    values={attribution_channel_totals[0]}
    colors={channel_colors}
    extra_values={attribution_residual_totals[0]}
    disable_extra={!(attribution_residual || attribution_single_channel == "res")}
    bind:state={state}
    height={css_multiply(video_height, 1/3)}
    width="100%"
    quantile_to_overflow={chart_quantile_to_overflow}
    max_overflow={chart_max_overflow}
    fixed_upper_value={chart_fixed_upper_value}
    fixed_lower_value={chart_fixed_lower_value}
  /> <!-- {css_multiply(video_width, 1.5)}
