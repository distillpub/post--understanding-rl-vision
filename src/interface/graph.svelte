<script>
  ...
  const format_value_function = function(n) {
        ...
        {#if typeof(series[index][state.position]) !== "undefined"}
          <div
            ...
            style="position: absolute;
                   ...
                   left: 3px;
                   margin-bottom: {show_extra_help_text ? '-1.25em' : '-0.75em'};
                   padding: 0.25em;
                   color: {colors[index]};
                   line-height: 1.1em;"
          >
            {@html titles[index]}
            {#if show_extra_help_text && titles[index] === "Value function"}
              <br>
              <span style="font-weight: normal; font-size: 0.75em; color: gray;">Estimated discounted win probability</span>
            {/if}
          </div>
          <div
            class="label"
            style="position: absolute;
                   ...
                   padding: 0.25em;"
          >
            {@html (titles[index].toLowerCase().indexOf("value") === -1 ? format_number : format_value_function)(series[index][state.position])}
          </div>
        {/if}
      </div>
    {/each}
